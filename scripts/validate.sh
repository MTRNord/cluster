...
kubeconform_config=("-ignore-missing-schemas" "-schema-location" "default" "-schema-location" "/tmp/flux-crd-schemas" "-verbose")
...
find ./clusters -maxdepth 2 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file; do
    kubeconform "${kubeconform_flags[@]}" "${kubeconform_config[@]}" "${file}" || echo "WARN - Validation failed for ${file}, continuing..."
done
...
find . -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file; do
    ...
    kustomize build "${file/%$kustomize_config/}" "${kustomize_flags[@]}" |
        kubeconform "${kubeconform_flags[@]}" "${kubeconform_config[@]}" || echo "WARN - Validation failed for ${file/%$kustomize_config/}, continuing..."
done
