...
def generate_csv(output_file: str = "hetzner_pricing.csv"):
    ...
    price_per_gb_month_net = volume_pricing.get("price_per_gb_month", {}).get("net")
    if price_per_gb_month_net is None:
        # Fall back to a hardcoded default but warn the user that API data was unavailable
        fallback_price_per_gb_month_net = "0.052"
        print(
            "Warning: Volume pricing (pricing.volume.price_per_gb_month.net) "
            "not returned by Hetzner API; using fallback net price_per_gb_month="
            f"{fallback_price_per_gb_month_net} EUR/GB/month",
            file=sys.stderr,
        )
        price_per_gb_month_net = fallback_price_per_gb_month_net
    volume_monthly = parse_hourly_price(price_per_gb_month_net)
